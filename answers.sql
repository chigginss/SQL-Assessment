...
SELECT name, SUM(num_cupcakes) FROM cupcakes LEFT JOIN orders ON cupcakes.id = orders.cupcake_id GROUP BY name;
...
SELECT email FROM customers JOIN orders ON orders.customer_id = customers.id GROUP BY email, num_cupcakes HAVING SUM(num_cupcakes) >= 100 ORDER BY email DESC;
